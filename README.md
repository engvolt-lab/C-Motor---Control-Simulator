...
flowchart TD
    %% Styling Classes
    classDef input fill:#1e293b,stroke:#38bdf8,stroke-width:1.8px,color:#f8fafc;
    classDef control fill:#0f172a,stroke:#38bdf8,stroke-width:1.8px,color:#f8fafc;
    classDef plant fill:#0f172a,stroke:#34d399,stroke-width:1.8px,color:#f8fafc;
    classDef sensor fill:#0f172a,stroke:#c084fc,stroke-width:1.8px,color:#f8fafc;
    classDef safety fill:#1f1315,stroke:#f87171,stroke-width:1.8px,color:#f8fafc;
    classDef output fill:#1e293b,stroke:#fbbf24,stroke-width:1.8px,color:#f8fafc;

    OP["<b>Operator & Scenario Inputs</b><br/>• Target RPM (0–3000)<br/>• Load Torque (0.01–0.25 N·m)<br/>• Fault Injections / E-Stop"]:::input

    subgraph MC ["<b>MotorController (Real-Time Orchestrator — dt = 10 ms)</b>"]
        direction TB

        subgraph SetpointShaping ["Setpoint Conditioning"]
            SLEW["<b>Slew Rate Limiter</b><br/>Ramp: ±800 RPM/s<br/><i>Prevents inrush trips</i>"]:::control
            SUM(("<b>Σ</b><br/>Error<br/><i>e(k)=r-y</i>")):::control
        end

        subgraph ControlCore ["Regulation & Power Interlock"]
            PID["<b>PIDController</b><br/>• P: Kp · e(k) [0.010]<br/>• I: Ki ∫e dt (Clamp [-200, 200])<br/>• D: Kd · Δe/Δt (Kick suppression)<br/>• Voltage Clamp: [0, 48] V"]:::control
            GATE{"<b>Safety Interlock Gate</b><br/>Normal: Pass V_cmd<br/>Tripped: Cut to 0.0 V"}:::safety
        end

        subgraph PlantDynamics ["<b>DC Motor Physics (Plant)</b>"]
            ELEC["<b>Electrical Dynamics</b><br/>Back-EMF: V_back = Ke · ω<br/>Current: I = (V_in - V_back) / R"]:::plant
            MECH["<b>Rotational Mechanics</b><br/>Torque: τ = Kt · I<br/>dω/dt = (τ - τ_load - Bω) / J"]:::plant
            THERM["<b>Lumped Thermal Model</b><br/>Joule Loss: P = I²R<br/>dT/dt = (P·Rth - ΔT) / τ_th"]:::plant
        end

        subgraph FeedbackPath ["Feedback Loop"]
            ENC["<b>Incremental Encoder</b><br/>• Resolution: 1024 PPR<br/>• Noise: ±0.5 RPM<br/>• Wire Break Detect: 0.0 RPM"]:::sensor
        end

        subgraph Protection ["Supervisory Protection"]
            SAFE["<b>SafetySystem (Latched Protection)</b><br/>• Overcurrent: I > 15.0 A<br/>• Overtemperature: T > 80.0 °C<br/>• Overspeed: ω > 3500 RPM<br/>• Encoder Sensor Loss<br/>• Hardware Emergency Stop"]:::safety
        end
    end

    OUT["<b>Real-Time Telemetry Interface</b><br/>• 0.5s ANSI Terminal Table<br/>• Interactive REPL Shell<br/>• Zero-Install HTML5 Visualizer"]:::output

    %% Signal Connections
    OP -->|Target Speed r| SLEW
    SLEW -->|Conditioned Setpoint| SUM
    SUM -->|Speed Error e| PID
    PID -->|V_cmd [0-48V]| GATE
    GATE -->|V_in| ELEC
    ELEC -->|Torque τ| MECH
    ELEC -->|Loss I²R| THERM
    MECH -->|True Speed ω| ENC
    ENC -->|Measured Speed y(k)| SUM

    %% Telemetry to Supervisory Protection
    ELEC -.->|Current I| SAFE
    THERM -.->|Temp T| SAFE
    MECH -.->|True Speed ω| SAFE
    ENC -.->|Encoder Health| SAFE
    SAFE ==>|Trip Signal: LATCH & CUT POWER| GATE

    %% Real-time telemetry sink
    MC ==>|Stream: Time, Target, Actual, Error, I, T, Status| OUT
```
